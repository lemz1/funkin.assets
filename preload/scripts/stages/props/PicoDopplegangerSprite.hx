...
class PicoDopplegangerSprite extends FlxAtlasSprite
{
  ...
  public function doAnim(_suffix:String, shoot:Bool = false, explode:Bool = false, timerManager:FlxTimerManager)
  {
    ...
    new FlxTimer(timerManager).start(0.3, _ -> {
      cutsceneSounds = FunkinSound.load(Paths.sound('cutscene/picoGasp'), 1.0, false, true, true);
    });
    ...
    if (shoot == true)
    {
      ...
      new FlxTimer(timerManager).start(10.33, _ -> {
        cutsceneSounds = FunkinSound.load(Paths.sound('cutscene/picoSpin'), 1.0, false, true, true);
      });
    }
    else
    {
      if (explode == true)
      {
        ...
        new FlxTimer(timerManager).start(8.75, _ -> {
          cutsceneSounds = FunkinSound.load(Paths.sound('cutscene/picoExplode'), 1.0, false, true, true);
        });
      }
      else
      {
