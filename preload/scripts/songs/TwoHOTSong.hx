...
class TwoHOTSong extends Song
{
  ...
  function getNextCanWithState(desiredState:Int):SpraycanAtlasSprite
  {
  ...
  function darkenStageProps()
  {
    ...
    for (stageProp in PlayState.instance.currentStage.members)
    {
      ...
      {
        if (cast(stageProp, StageProp).name == "bf"
          || cast(stageProp, StageProp).name == "dad" || cast(stageProp, StageProp).name == "gf") // This refers to the player.
        {
          // Exclude.
          continue;
        }
      ...
      // Select cans.
      if (Std.isOfType(stageProp, SpraycanAtlasSprite) && spawnedCans.contains(cast(stageProp, SpraycanAtlasSprite)))
      {
        // Exclude.
        continue;
      }
  ...
  function blackenStageProps()
  {
    ...
    for (stageProp in PlayState.instance.currentStage.members)
    {
      ...
      if (Std.isOfType(stageProp, StageProp))
      {
        if (cast(stageProp, StageProp).name == "bf") // This refers to the player.
        {
          // Exclude.
          continue;
        }
  ...
  function spawnImpactParticle()
  {
    ...
    impactParticle.animation.finishCallback = function(_) {
      impactParticle.kill();
    };
  }
