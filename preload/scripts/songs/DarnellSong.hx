...
class DarnellSong extends Song
{
  ...
  function introCutscene()
  {
    ...
    new FlxTimer().start(0.7, function(tmr) {
      ...
      FlxTween.tween(bgSprite, {alpha: 0}, 2,
        {
          startDelay: 0.3,
  ...
  function darkenStageProps()
  {
    ...
    for (stageProp in PlayState.instance.currentStage.members)
    {
      ...
      {
        if (cast(stageProp, Bopper).name == "bf"
          || cast(stageProp, Bopper).name == "dad" || cast(stageProp, Bopper).name == "gf") // This refers to the player.
        {
          // Exclude.
          continue;
        }
