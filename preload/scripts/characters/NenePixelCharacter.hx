...
class NenePixelCharacter extends SparrowCharacter
{
  ...
  var abotHead:FunkinSprite;
  var abotSpeaker:FunkinSprite;
   ...
   */
  static final VULTURE_THRESHOLD = 0.25 * 2;
   ...
   */
  static final STATE_PRE_RAISE = 1;
   ...
   */
  static final MIN_BLINK_DELAY:Int = 3;

  static final MAX_BLINK_DELAY:Int = 7;
  ...
  public function addSunsetShaders()
  {
    ...
    abotSpeaker.animation.onFrameChange.add(function(_, _, _) {
  ...
  {
    if (abotSpeaker != null) abotSpeaker.animation.play("danceLeft", true);
    ...
    switch (currentState)
    {
      case STATE_DEFAULT:
        if (hasDanced)
        {
          ...
          if (abot != null) abot.animation.play("danceRight", true);
        }
        else
        {
          playAnimation('danceLeft', true);
          if (abot != null) abot.animation.play("danceLeft", true);
        }
        hasDanced = !hasDanced;
  ...
  override function onAnimationFrame(name:String = "", frameNumber:Int = -1, frameIndex:Int = -1)
  {
