...
class PicoPlayerCharacter extends MultiSparrowCharacter
{
  ...
  override function onAnimationFinished(name:String)
  {
    ...
    if (name == 'shootMISS' && PlayState.instance.health > 0.0 && !PlayState.instance.isPlayerDying)
    {
      ...
      picoFlicker = FlxFlicker.flicker(this, 1, 1 / 30, true, true, function(_) {
        picoFlicker = FlxFlicker.flicker(this, 0.5, 1 / 60, true, true, function(_) {
          picoFlicker = null;
        });
      });
    }
  ...
  override function onUpdate(event:UpdateScriptEvent)
  {
    ...
    if (deathSpriteRetry != null) trace('deathSpriteRetry: ' + deathSpriteRetry.visible + ' (' + deathSpriteRetry.x + ', ' + deathSpriteRetry.y + ')');
  }
