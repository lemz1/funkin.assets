...
class PicoBlazinCharacter extends AnimateAtlasCharacter
{
  ...
  function afterPicoDeathGutPunchIntro(_:Null<FlxTimer>):Void
  {
  ...
  function doDeathConfirm():Void
  {
    ...
    picoDeathConfirm.animation.finishCallback = (_) -> {
  ...
  override function onNoteMiss(event:NoteScriptEvent)
  {
    ...
    if (willMissBeLethal(event.healthChange))
    {
      playHitLowAnim();
      return;
    }
  ...
  function willMissBeLethal(healthChange:Float):Bool
  {
    return (PlayState.instance.health + healthChange) <= 0.0;
  }
